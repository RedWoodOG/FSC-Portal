...
import 'package:portal_offline/features/auth/auth_state.dart';
import 'knowledge_category_seed.dart';
...
Future<void> seedDatabase(AppDatabase db) async {
  ...
  // Seed industry briefings only if none exist (preserve user-acknowledged state)
  final existingBriefings = await db.getLatestIndustryBriefing(limit: 1);
  if (existingBriefings.isEmpty) {
    await db.insertIndustryBriefing(IndustryBriefingCompanion.insert(
    ...
    await db.insertIndustryBriefing(IndustryBriefingCompanion.insert(
      ...
      previewImage: const Value(
          'assets/images/news_kisan.jpg'), // Proxy: Currency counter
    ...
    await db.insertIndustryBriefing(IndustryBriefingCompanion.insert(
      ...
      source: 'Fenco',
      previewImage: const Value('assets/images/news_AV_2.webp'), // Proxy: Vault
  ...
  if (existingClients.isNotEmpty &&
    ...
    // But we still check for Prosperity enrichment and team updates
    await _seedProsperityLocations(db);
    await _seedTeamMembers(db);

    // Seed knowledge base categories and tags (always check)
    await seedKnowledgeCategories(db);
    await seedKnowledgeTags(db);
    return;
  }

  Log.info('Seeding database with demo data...');

  // Insert Default User if not exists (with hashed password)
  if (existingUsers.isEmpty) {
    final salt = AuthState.generateSalt('jwhite');
    final hash = AuthState.hashPassword('password123', salt);
    ...
    Log.info('- Default User (Joseph White) created with hashed credentials');
  } else {
  ...
  // Seed knowledge base categories and tags
  await seedKnowledgeCategories(db);
  await seedKnowledgeTags(db);
  ...
  if (existingClients.isNotEmpty) {
    ...
    // Insert Clients
    final rbfcuId = await db.into(db.clients).insert(
  ...
  if (weatherCheck == null) {
    await db.insertWeather(WeatherSnapshotCompanion.insert(
      region: 'North Region',
      temperature: 72,
      condition: 'Clear',
      fetchedAt: DateTime.now(),
  ...
  if (existingTraffic == null) {
    await db.insertTraffic(TrafficSnapshotCompanion.insert(
  ...
  if (workCallsCount == 0) {
    ...
    // 6 Open calls
    for (int i = 0; i < 6; i++) {
      await db.insertWorkCall(WorkCallsCompanion.insert(
        status: 'open',
    ...
    for (int i = 0; i < 10; i++) {
      ...
      await db.insertWorkCall(WorkCallsCompanion.insert(
        status: 'completed',
        completedAt:
            Value(weekStart.add(Duration(days: dayOffset, hours: 10 + i))),
  ...
  if (existingWorkOrders.isEmpty) {
    ...
    if (sites.isNotEmpty) {
      ...
      // Seed machine profiles (canonical equipment identity)
      final existingProfiles = await db.getAllMachineProfiles();
      int magnerProfileId;
      // ignore: unused_local_variable
      int dieboldProfileId;
      // ignore: unused_local_variable
      int honProfileId;

      if (existingProfiles.isEmpty) {
        magnerProfileId = await db.insertMachineProfile(
          MachineProfilesCompanion.insert(
            equipmentType: 'counter',
            manufacturer: 'Magner',
            model: '305 CDS',
            aliases: const Value('["Magner 305", "305CDS", "Mag 305"]'),
            searchKeys: const Value('magner 305 cds counter coin sorter'),
            description: const Value(
                'High-speed coin counter/sorter with dual pocket design'),
          ),
        );
        dieboldProfileId = await db.insertMachineProfile(
          MachineProfilesCompanion.insert(
            equipmentType: 'atm',
            manufacturer: 'Diebold Nixdorf',
            model: 'DN Series 200',
            aliases: const Value('["Diebold 200", "DN200", "Nixdorf 200"]'),
            searchKeys:
                const Value('diebold nixdorf dn 200 atm cash dispenser'),
            description:
                const Value('Full-function ATM with cash recycling capability'),
          ),
        );
        honProfileId = await db.insertMachineProfile(
          MachineProfilesCompanion.insert(
            equipmentType: 'lock',
            manufacturer: 'Kaba Mas',
            model: 'X-10',
            aliases: const Value('["Kaba X10", "X10 Lock", "Mas X-10"]'),
            searchKeys: const Value('kaba mas x-10 x10 lock high security'),
            description: const Value(
                'High-security electronic combination lock for vault doors'),
          ),
        );
        Log.info('- Machine profiles seeded (3 items)');
      } else {
        // Use existing profile IDs
        magnerProfileId = existingProfiles
            .firstWhere(
              (p) => p.manufacturer == 'Magner',
              orElse: () => existingProfiles.first,
            )
            .id;
        dieboldProfileId = existingProfiles
            .firstWhere(
              (p) => p.manufacturer == 'Diebold Nixdorf',
              orElse: () => existingProfiles.first,
            )
            .id;
        honProfileId = existingProfiles
            .firstWhere(
              (p) => p.manufacturer == 'Kaba Mas',
              orElse: () => existingProfiles.first,
            )
            .id;
      }

      // Insert sample equipment (linked to machine profiles)
      final equipmentId = await db.into(db.equipment).insert(
            EquipmentCompanion.insert(
              siteId: firstSite.id,
              equipmentType: 'counter',
              machineProfileId: Value(magnerProfileId),
      ...
      Log.info('- Equipment seeded (1 item, linked to machine profile)');
      ...
      final workOrderId = await db.into(db.workOrders).insert(
            WorkOrdersCompanion.insert(
              ...
              descriptionOfWork:
                  const Value('Coin sorter not dispensing properly'),
      ...
      final completedWorkOrderId = await db.into(db.workOrders).insert(
            WorkOrdersCompanion.insert(
              ...
              descriptionOfWork:
                  const Value('Routine maintenance and calibration'),
              createdAt: lastWeek,
      ...
      final workPerformedId = await db.into(db.workPerformed).insert(
            WorkPerformedCompanion.insert(
              ...
              resolution:
                  const Value('Equipment operating normally after calibration'),
...
Future<void> _ingestKnowledgeEntries(AppDatabase db) async {
  ...
  try {
    // Upsert-based ingestion: preserve existing data, only update changed entries
    final existingEntries = await db.getAllKnowledgeEntries();
    Log.info('Current entries in database: ${existingEntries.length}');
    Log.info('Starting upsert ingestion from: $knowledgeDir');
    ...
    for (final file in mdFiles) {
      try {
        ...
        title ??= path
            ...
            .replaceAll('_', ' ')
            .replaceAll('-', ' ');
        category ??= _guessCategoryFromPath(file.path);
        final equipmentType = _guessEquipmentTypeFromPath(file.path);
        ...
        await db.insertOrReplaceKnowledge(
          KnowledgeEntriesCompanion.insert(
            ...
            sourceType: metadata?['source_type']?.toString() ?? 'manual_import',
            sourceFile: metadata?['source_file']?.toString() ??
                path.basename(file.path),
            version: metadata?['version']?.toString() ?? '1.0.0',
            status: metadata?['status']?.toString() ?? 'active',
...
Future<void> _seedTeamMembers(AppDatabase db) async {
  ...
  for (final member in teamMembers) {
    final salt = AuthState.generateSalt(member['username']!);
    final hash = AuthState.hashPassword('changeme', salt);
