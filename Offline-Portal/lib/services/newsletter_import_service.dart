...
class NewsletterImportService {
  ...
  Future<int> importFromPdf(String pdfPath) async {
    ...
    for (int i = 0; i < pdfDoc.pages.count; i++) {
      textBuffer.writeln(PdfTextExtractor(pdfDoc).extractText(startPageIndex: i, endPageIndex: i));
    }
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (_isTitle(line, i < lines.length - 1 ? lines[i + 1].trim() : '')) {
        ...
        if (currentTitle != null && currentBodyLines.isNotEmpty) {
          announcements.add(_createAnnouncement(
            title: currentTitle,
        ...
        if (currentTitle == null) {
          currentTitle = line.length > 60 ? '${line.substring(0, 60)}...' : line;
          currentBodyLines.add(line);
        }
      }
    }
  ...
  String? _extractActionLabel(String line) {
    ...
    if (lower.contains('select benefits') || lower.contains('select')) return 'SELECT BENEFITS';
    if (lower.contains('view policy') || lower.contains('view')) return 'VIEW POLICY';
    if (lower.contains('learn more')) return 'LEARN MORE';
    if (lower.contains('read more')) return 'READ MORE';
    if (lower.contains('register') || lower.contains('sign up')) return 'REGISTER';
